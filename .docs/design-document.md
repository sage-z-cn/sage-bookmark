...
│   │   ├── 新建（文件夹/书签）
│   │   ├── 剪切 / 复制 / 粘贴
│   │   ├── 编辑（书签：Modal 编辑标题+URL / 文件夹：行内重命名）
│   │   ├── 分享
│   │   ├── 删除
│   │   ├── 搜索（点击展开搜索栏）
│   │   └── 视图切换（大图标/列表）
...
| 面包屑路径 | 显示完整路径，如 `书签栏 > 开发 > 前端框架`，点击任意层级可跳转；宽度不足时采用中间省略策略（见 §3.7） |
...
| 编辑       | 工具栏按钮 / F2            | 书签：弹出 Modal 编辑标题和 URL；文件夹：进入行内编辑模式修改标题 |
| 分享       | 工具栏按钮                 | 复制书签 URL 或导出选中项为 HTML                                  |
| 删除       | 工具栏按钮 / Delete 键     | 删除选中项目，弹出确认对话框                                      |
| 搜索       | 工具栏按钮 / 快捷键 Ctrl+F | 点击展开/收起搜索栏，在当前文件夹及其子文件夹中搜索               |
| 视图切换   | 工具栏按钮                 | 切换大图标模式 / 详细列表模式                                     |

#### 2.2.3 搜索栏区域

搜索栏位于工具栏下方，点击工具栏「搜索」按钮后展开显示，再次点击或按 Esc 收起。

| 功能项     | 说明                                                     |
| ---------- | -------------------------------------------------------- |
| 搜索输入框 | 输入关键词进行搜索，支持标题和 URL 模糊匹配，实时过滤    |
| 搜索范围   | 默认在当前文件夹及其子文件夹中搜索，可切换为全局搜索模式 |
| 高亮显示   | 搜索结果中高亮匹配的关键词                               |
| 清空按钮   | 一键清空搜索关键词，返回正常浏览模式                     |
...
| 放置       | 点击 Dock 栏右侧「放置」按钮，将所有暂存项目批量移动到当前目录，按暂存顺序排列                        |
| 取消暂存   | 点击 Dock 栏右侧「取消」按钮，清空暂存列表但不执行移动操作，项目保留在原位置                          |
...
```
  宽度充足 — 完整路径：
┌──────────────────────────────────────────────────────────┐
│  [←] [→] [↻]  书签栏 > 开发 > 前端框架                   │  ← 地址栏 (56px)

  宽度不足 — 中间省略（Windows 风格）：
┌──────────────────────────────────────────────────────────┐
│  [←] [→] [↻]  书签栏 > ... > 按钮组件                    │  ← 地址栏 (56px)
├──────────────────────────────────────────────────────────┤
│  [+新建] [✂剪切] [📋复制] [✏编辑] [🔗分享] [🗑删除] [🔍] │  ← 工具栏 (44px)
│                                        [⊞大图标][≡列表]  │
├──────────────────────────────────────────────────────────┤
│  ┌──────────────────────────────────────────────────┐    │  ← 搜索栏区域（点击搜索按钮展开，40px）
│  │ 🔍 搜索...                              [范围▼]  │    │
...
├──────────────────────────────────────────────────────────┤
│  📁 React  📁 Vue  🔗 Docs  📁 Next  ...  ┌────────┐    │  ← Dock 暂存栏（拖拽时显示）
│                                          │ 📥 放置 │    │
│                                          │ ✕ 取消 │    │
│                                          └────────┘    │
└──────────────────────────────────────────────────────────┘
...
```
用户选中一个书签 → 点击工具栏「编辑」/ F2 / 右键菜单「编辑」
    → 弹出 Modal 对话框
    → 标题输入框：预填当前标题
    → URL 输入框：预填当前 URL，使用等宽字体（JetBrains Mono）显示
    → 用户修改标题和/或 URL
    → 点击「保存」
        → 前端校验 URL 格式（非空、合法 URL 协议头）
        → 校验失败 → URL 输入框下方显示错误提示
        → 校验通过 → 调用 chrome.bookmarks.update(id, { title, url })
        → 关闭 Modal
        → 内容区刷新显示
        → 显示 message.success：「书签已更新」
    → 点击「取消」或按 Esc → 关闭 Modal，不执行修改
```
...
```
用户选中一个文件夹 → 点击工具栏「编辑」/ F2 / 右键菜单「编辑」
    → 直接进入行内编辑模式（不弹出 Modal）
    → 标题变为可编辑输入框，预填当前名称
    → 回车确认 → 调用 chrome.bookmarks.update(id, { title })
    → Esc 取消 → 恢复原名称
```
...
```
用户点击 Dock 栏右侧「取消」按钮
    → 弹出确认对话框：「确定取消暂存 N 个项目？项目将保留在原位置。」
    → 用户确认
        → 清空 Dock 暂存列表
        → 清除 chrome.storage.session 中的暂存数据
        → 内容区中已暂存项目的半透明状态恢复
        → 显示 message.info：「已取消暂存 N 个项目」
        → Dock 栏自动隐藏
    → 用户取消
        → 关闭确认对话框，无其他操作
```
...
Dock 栏右侧提供两个操作按钮，垂直排列：「放置」在上，「取消」在下。

```
┌──────────────────────────────────────────────────────────────┐
│ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐        ┌────────┐          │
│ │📁   │ │🔗   │ │📁   │ │🔗   │        │ 📥 放置 │ ← 上     │
│ │React│ │Docs │ │Vue  │ │MDN  │        │ ✕ 取消 │ ← 下     │
│ │  ×  │ │  ×  │ │  ×  │ │  ×  │        └────────┘          │
│ └─────┘ └─────┘ └─────┘ └─────┘                             │
└──────────────────────────────────────────────────────────────┘
```
...
| 属性     | 值                                              |
| -------- | ----------------------------------------------- |
| 按钮样式 | Ant Design `Button` type="default" size="small" |
| 图标     | `✕` 或 `X` icon                                 |
| 文案     | 「取消」                                        |
| 位置     | 「放置」按钮下方，间隔 8px                      |
| 功能     | 清空 Dock 暂存列表，不执行任何移动操作          |
| 交互     | 点击后弹出确认：「确定取消暂存 N 个项目？」     |
...
当地址栏宽度不足以完整显示所有路径层级时，采用 Windows 资源管理器风格的中间省略策略，保留路径的起始和结尾部分，中间被隐藏的层级以可点击的省略号替代。

#### 省略策略

```
完整路径：书签栏 > 开发 > 前端框架 > React > 组件库 > 按钮组件

宽度充足 — 显示全部：
  书签栏 > 开发 > 前端框架 > React > 组件库 > 按钮组件
...
1. 始终保留第一个片段（根节点）和最后一个片段（当前节点）
2. 从中间层级开始移除，直到总宽度 ≤ 可用宽度
3. 被移除的中间层级替换为一个可点击的 `...` 省略号
4. 如果根节点或当前节点本身超宽，对单个片段内部做 `text-overflow: ellipsis` 截断
5. 面板宽度动态变化时（ResizeObserver）自动重新计算

#### 交互行为

| 交互         | 行为                                              |
| ------------ | ------------------------------------------------- |
| 点击省略号   | 弹出下拉面板，展示完整路径的所有被隐藏层级        |
| 下拉面板项   | 每个层级显示文件夹图标 + 名称，点击跳转到对应层级 |
| 当前层级高亮 | 下拉面板中当前所在层级使用 indigo 文字色标记      |
| 点击可见片段 | 直接跳转到该层级（同现有面包屑行为）              |
| 失焦 / ESC   | 关闭下拉面板                                      |
| hover 省略号 | 背景变色，提示可点击                              |
...
| 属性              | 值                                               |
| ----------------- | ------------------------------------------------ |
| 路径片段字体      | DM Sans, 14px, colorText (#0A0A0A)               |
| 分隔符            | `>` 图标，colorTextQuaternary (#9C9C9C)          |
| 省略号文字        | `...`，colorTextSecondary (#6B6B6B)              |
| 省略号 hover 背景 | `#F5F5F7`，圆角 4px                              |
| 省略号点击区域    | 水平 padding 8px，垂直 padding 4px               |
| 下拉面板组件      | Ant Design Dropdown                              |
| 下拉面板背景      | Surface (#FFFFFF)，12px 圆角，shadow-lg          |
| 下拉面板项行高    | 40px                                             |
| 下拉面板项 hover  | 背景 `#F5F5F7`                                   |
| 当前层级文字色    | colorPrimary (#6366F1)                           |
| 单片段截断        | `text-overflow: ellipsis`，`white-space: nowrap` |

#### 动画规格

| 动画                | 时长  | 缓动     |
| ------------------- | ----- | -------- |
| 下拉面板展开        | 150ms | ease-out |
| 下拉面板收起        | 100ms | ease-in  |
| 省略号 hover 背景   | 150ms | ease     |
| 路径片段 hover 背景 | 100ms | ease     |

#### 溢出检测机制

```typescript
// 使用 ResizeObserver 监听面包屑容器宽度变化
// 每个路径片段渲染后通过 getBoundingClientRect 测量实际宽度
// 使用 requestAnimationFrame 节流避免频繁重排
...
| 场景                     | 处理方式                              |
| ------------------------ | ------------------------------------- |
| 路径只有 1 层            | 不显示省略号，直接显示该层            |
| 路径只有 2 层            | 显示 `根 > 当前`，不省略              |
| 路径 3 层且溢出          | 显示 `根 > ... > 当前`                |
| 所有片段都超出           | 至少保留 `根 > ... > 当前`            |
| 单个片段极长（>50 字符） | 片段内 `text-overflow: ellipsis` 截断 |
| 面板宽度动态变化         | ResizeObserver 自动重新计算           |
...
│   │   │   │   ├── Breadcrumb/
│   │   │   │   │   ├── index.tsx          # 面包屑容器
│   │   │   │   │   ├── BreadcrumbItem.tsx # 单个路径片段
│   │   │   │   │   └── OverflowMenu.tsx   # 省略号展开下拉面板
│   │   │   ├── Toolbar/            # 工具栏组件
│   │   │   │   ├── index.tsx
│   │   │   │   ├── SearchButton.tsx      # 搜索按钮
│   │   │   │   └── ViewSwitch.tsx
│   │   │   ├── SearchBar/          # 搜索栏区域
│   │   │   │   ├── index.tsx       # 搜索栏容器（展开/收起逻辑）
...
│   │   │   ├── useBreadcrumbOverflow.ts # 面包屑溢出检测
...
| `useDock`               | 暂存/取回/放置、去重校验、持久化恢复、循环移动校验 |
...
| E2E-13  | Dock 放置      | Dock 栏暂存 3 项 → 切换目录 → 点击「放置」    | 3 项全部移动到当前目录            |
