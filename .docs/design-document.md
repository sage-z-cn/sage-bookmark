...
Sage Bookmark 是一款基于 Manifest V3 标准开发的浏览器扩展插件，提供类似 Windows 资源管理器的收藏夹管理体验。用户可以在整页选项页面中以直观的图形界面管理浏览器书签，支持大图标/列表视图切换、拖拽移动、批量操作、框选等高级交互功能。
...
| 构建 | Vite + CRXJS | Vite 5.x / CRXJS latest | CRXJS 提供 Chrome 扩展 HMR 支持 |
...
│   │   ├── index.html
│   │   ├── main.tsx
│   │   └── App.tsx                # 简单入口 → 打开 Options Page
│   ├── background/                # Service Worker
│   │   └── index.ts               # 事件监听、扩展安装初始化
...
type BookmarkAction =
  ...
  | { type: 'SELECT'; payload: { ids: string[]; mode: 'set' | 'toggle' | 'range' } }
...
// 伪代码：vite.config.ts 关键配置
{
  plugins: [
    react(),
    crx({ manifest: './public/manifest.json' }),  // CRXJS 插件
  ],
  build: {
    rollupOptions: {
      input: {
...
class BookmarkService {
  ...
  // CRUD
  async createBookmark(parentId: string, title: string, url: string): Promise<AppBookmarkNode>
  ...
  async moveItems(ids: string[], targetParentId: string, targetIndex?: number): Promise<void>

  // 剪贴板
  async pasteItems(clipboard: ClipboardData, targetParentId: string): Promise<void>
...
> **注意**：`favicon` 权限在 Manifest V3 中需要显式声明才能使用 `chrome://favicon/`。Edge 和 Chrome 均支持。
...
| Service Worker 生命周期限制 | 后台事件处理 | Options Page 是持久页面，不受影响 |
...
- `manifest.json` 中的 `version` 字段与 `package.json` 保持同步
