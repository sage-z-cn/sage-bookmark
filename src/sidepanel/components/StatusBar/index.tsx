...
export default function StatusBar() {
  ...
  const bookmarkCount = currentItems.filter(i => i.type === 'bookmark').length
  ...
  return (
    <div className={styles.statusBar}>
      <span className={styles.stat}>
        {totalCount} 个项目
        {folderCount > 0 && `（${folderCount} 个文件夹`}{folderCount > 0 && bookmarkCount > 0 ? '，' : ''}
        {bookmarkCount > 0 ? `${bookmarkCount} 个书签）` : folderCount > 0 ? '）' : ''}
      </span>
      {selectedIds.size > 0 && (
